--thanks to stefanuk12 on github for fixing the crash bug
local GenGUID = false
local themes = {
...
local library = {}
local page = {}
local function MakeGuiName(a) --math.random(10,20)
	if GenGUID == true then
		return game:GetService("HttpService"):GenerateGUID()
	elseif GenGUID == false then
		if GetGuiName == nil then
			local xo = {}
			for i = 1, a do
				xo[i] = string.char(math.random(32, 126))
			end
			getgenv().GetGuiName = table.concat(xo)
			return table.concat(xo)
		else
			return getgenv().GetGuiName
		end
	else
		return tostring(GenGUID)
	end
end
...
do
	...
	function library.new(data)
		local title = data[1] or "nil"

		local container = utility:Create("ScreenGui", {
			Name = MakeGuiName(math.random(10,20)),
			Parent = game.CoreGui
		}, {
	...
	function library:Notify(data)
		local title = data[1] or "nil"
		local text = data[2] or "nil"
		local callback = data[3] or function() end
	...
	function section:addButton(data)
		local this = {}
		this.title = data[1] or "nil text"
		this.callback = data[2] or function() end
	...
	function section:addToggle(data)
		local this = {}
		this.title = data[1] or "nil"
		this.toggled = data[2] or false
		this.callback = data[3] or function() end
	...
	function section:addKeybind(data)
		local this = {}
		this.title = data[1] or "nil"
		this.key = data[2] or Enum.KeyCode.Unknown
		this.callback = data[3] or function() end
		this.changedCallback = data[4] or function() end
		...
		local keybind = utility:Create("ImageButton", {
			...
			utility:Create("ImageLabel", {
				...
				utility:Create("TextLabel", {
					...
					ZIndex = 3,
					Font = Enum.Font.GothamSemibold,
					Text = game:GetService("UserInputService"):GetStringForKeyCode(this.key),
					TextColor3 = themes.TextColor,
					TextSize = 11
				})
			})
	...
	function section:addColorPicker(data)
		local this = {}
		this.title = data[1] or "nil"
		this.default = data[2] or Color3.new(255, 150, 150)
		this.callback = data[3] or function() end
		...
		canvas.MouseButton1Down:Connect(function()
			...
			while draggingCanvas do
				...
				local x, y = game.Players.LocalPlayer:GetMouse().X, game.Players.LocalPlayer:GetMouse().Y
		...
		color.MouseButton1Down:Connect(function()
			...
			while draggingColor do
				...
				hue = 1 - math.clamp(1 - ((game.Players.LocalPlayer:GetMouse().X - colorPosition.X) / colorSize.X), 0, 1)
				this.color3 = Color3.fromHSV(hue, sat, brightness)
	...
	function section:addSlider(data)
		local this = {}
		this.title = data[1] or "nil"
		this.default = tonumber(data[2]) or 0
		this.min = tonumber(data[3]) or -50
		this.max = tonumber(data[4]) or 50
		this.callback = data[5] or function() end

		this.precision = 0 --data.precision or 0
		this.value = nil --this.default
	...
	function section:addDropdown(data)
		...
		this.default = nil --data.default
		this.backuplist = nil --this.list
	...
	function section:updateKeybind(module)
		...
		if options.key ~= Enum.KeyCode.Unknown then
			...
			text.Text = game:GetService("UserInputService"):GetStringForKeyCode(options.key)
		else
			text.Text = "Unknown"
		end
	end
	...
	function section:updateSlider(module)
		...
		local percent = (game.Players.LocalPlayer:GetMouse().X - bar.AbsolutePosition.X) / bar.AbsoluteSize.X
